...
describe("ShoppingCartService", () => {
    ...
    describe("findById", () => {
        ...
        it("deve lançar AppError 404 quando carrinho não existe", async () => {
            ...
            await expect(cartService.findById(99)).rejects.toThrow(new AppError("Carrinho não encontrado", 404));
        });
    });
    describe("findByUserId", () => {
        ...
        it("deve lançar AppError 404 quando usuário não tem carrinho ativo", async () => {
            ...
            await expect(cartService.findByUserId(99)).rejects.toThrow(new AppError("Carriinho de Usuário não encontrado", 404));
        });
    });
    describe("update", () => {
        it("deve atualizar carrinho quando existe", async () => {
            const existingCart = makeShoppingCart();
            const updateData = { status: "CHECKED_OUT" };
        ...
        it("deve lançar AppError 404 quando carrinho não existe", async () => {
            vi.mocked(cartRepo.findById).mockResolvedValue(null);
            await expect(cartService.update(99, { status: "ABANDONED" })).rejects.toThrow(new AppError("Carrinho não encontrado", 404));
    ...
    describe("delete", () => {
        ...
        it("deve lançar AppError 404 quando carrinho não existe", async () => {
            vi.mocked(cartRepo.findById).mockResolvedValue(null);
            await expect(cartService.delete(99)).rejects.toThrow(new AppError("Carrinho não encontrado", 404));
    ...
    describe("create", () => {
        ...
        it("deve lançar AppError 409 quando usuário já tem carrinho ativo", async () => {
            ...
            await expect(cartService.create(validCartData)).rejects.toThrow(new AppError("Já tem um carrinho ativo", 409));
        ...
        it("deve validar que usuário não tem carrinho antes de criar", async () => {
            ...
            expect(cartRepo.findByUserId).toHaveBeenCalledBefore(cartRepo.create);
        });
    });
});
//# sourceMappingURL=ShoppingCartService.test.js.map