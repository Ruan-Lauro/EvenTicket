...
import { ShoppingCartItemService } from "../../../src/services/shoppingCartItemService.js";
...
describe("ShoppingCartItemService", () => {
    ...
    describe("addItem", () => {
        it("deve lançar AppError 404 quando assento não existe", async () => {
            ...
            await expect(cartItemService.addItem(10, 99)).rejects.toThrow(new AppError("Assento não encontrado", 404));
        ...
        it("deve lançar AppError 409 quando assento não está disponível", async () => {
            ...
            await expect(cartItemService.addItem(10, 1)).rejects.toThrow(new AppError("Assento indisponível", 409));
            expect(cartRepo.findByUserId).not.toHaveBeenCalled();
        });
        ...
        it("deve lançar AppError 409 quando assento já está no carrinho", async () => {
            ...
            await expect(cartItemService.addItem(10, 1)).rejects.toThrow(new AppError("Assento já adicionado ao carrinho", 409));
        ...
        it("deve lançar AppError 404 quando publicação do assento não existe", async () => {
            ...
            await expect(cartItemService.addItem(10, 1)).rejects.toThrow(new AppError("Publicação do assento não encontrada", 404));
            expect(cartItemRepo.create).not.toHaveBeenCalled();
        });
    ...
    describe("removeItem", () => {
        it("deve lançar AppError 404 quando item não existe", async () => {
            ...
            await expect(cartItemService.removeItem(10, 99)).rejects.toThrow(new AppError("Item não encontrado", 404));
        ...
        it("deve lançar AppError 403 quando carrinho não existe", async () => {
            ...
            await expect(cartItemService.removeItem(10, 1)).rejects.toThrow(new AppError("Acesso negado", 403));
        ...
        it("deve lançar AppError 400 quando carrinho não está ACTIVE", async () => {
            ...
            vi.mocked(cartItemRepo.findById).mockResolvedValue(item);
            vi.mocked(cartRepo.findById).mockResolvedValue(cart);
            await expect(cartItemService.removeItem(10, 1)).rejects.toThrow(new AppError("Não é possível remover itens de um carrinho finalizado", 400));
            expect(cartRepo.update).not.toHaveBeenCalled();
        });
    ...
    describe("listByCart", () => {
        ...
        it("deve lançar AppError 404 quando usuário não tem carrinho ativo", async () => {
            vi.mocked(cartRepo.findByUserId).mockResolvedValue(null);
            await expect(cartItemService.listByCart(99)).rejects.toThrow(new AppError("Nenhum carrinho ativo encontrado", 404));
...
//# sourceMappingURL=ShoppingCartItemService.test.js.map