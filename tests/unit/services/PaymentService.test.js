...
describe("PaymentService", () => {
    ...
    beforeEach(() => {
        ...
        paymentService = new PaymentService(paymentRepo, purchaseRepo, cartRepo, cartItemRepo, ticketRepo, seatRepo);
    });
    describe("initiate", () => {
        it("deve lançar AppError 404 quando compra não existe", async () => {
            vi.mocked(purchaseRepo.findById).mockResolvedValue(null);
            await expect(paymentService.initiate(99, "PIX", 10)).rejects.toThrow(new AppError("Compra não encontrada", 404));
        ...
        it("deve lançar AppError 403 quando carrinho não existe ou não pertence ao usuário", async () => {
            ...
            await expect(paymentService.initiate(1, "PIX", 10)).rejects.toThrow(new AppError("Acesso negado", 403));
            expect(paymentRepo.create).not.toHaveBeenCalled();
        });
        ...
        it("deve lançar AppError 409 quando pagamento já foi realizado", async () => {
            ...
            await expect(paymentService.initiate(1, "PIX", 10)).rejects.toThrow(new AppError("Compra já foi paga", 409));
            expect(paymentRepo.create).not.toHaveBeenCalled();
        });
        it("deve lançar AppError 409 quando pagamento está pendente", async () => {
            ...
            vi.mocked(cartRepo.findById).mockResolvedValue(cart);
            vi.mocked(paymentRepo.findByPurchaseId).mockResolvedValue(existingPayment);
            await expect(paymentService.initiate(1, "PIX", 10)).rejects.toThrow(new AppError("Pagamento já em andamento", 409));
            expect(paymentRepo.create).not.toHaveBeenCalled();
        });
        ...
        it("deve lançar AppError quando assento não existe durante confirmação", async () => {
            ...
            vi.spyOn(Math, "random").mockReturnValue(0.5);
            await expect(paymentService.initiate(1, "PIX", 10)).rejects.toThrow(new AppError(`Assento ${item.seatId} não encontrado`, 404));
        });
    });
    describe("cancel", () => {
        it("deve lançar AppError 404 quando compra não existe", async () => {
            vi.mocked(purchaseRepo.findById).mockResolvedValue(null);
            await expect(paymentService.cancel(99, 10)).rejects.toThrow(new AppError("Compra não encontrada", 404));
        });
        it("deve lançar AppError 403 quando carrinho não existe ou não pertence ao usuário", async () => {
            const purchase = makePurchase();
            vi.mocked(purchaseRepo.findById).mockResolvedValue(purchase);
            vi.mocked(cartRepo.findById).mockResolvedValue(null);
            await expect(paymentService.cancel(1, 10)).rejects.toThrow(new AppError("Acesso negado", 403));
        });
        it("deve lançar AppError 403 quando carrinho pertence a outro usuário", async () => {
            const purchase = makePurchase();
            const cart = makeShoppingCart({ userId: 99 });
        ...
        it("deve cancelar compra com pagamento PAID e fazer estorno", async () => {
            ...
            vi.mocked(paymentRepo.updateStatus).mockResolvedValue(makePayment({ status: "REFUNDED" }));
    ...
    describe("findByPurchaseId", () => {
        ...
        it("deve lançar AppError 404 quando compra não existe", async () => {
            vi.mocked(purchaseRepo.findById).mockResolvedValue(null);
            await expect(paymentService.findByPurchaseId(99, 10)).rejects.toThrow(new AppError("Compra não encontrada", 404));
        });
        it("deve lançar AppError 403 quando carrinho não existe ou não pertence ao usuário", async () => {
            const purchase = makePurchase();
            vi.mocked(purchaseRepo.findById).mockResolvedValue(purchase);
            vi.mocked(cartRepo.findById).mockResolvedValue(null);
            await expect(paymentService.findByPurchaseId(1, 10)).rejects.toThrow(new AppError("Acesso negado", 403));
        });
        it("deve lançar AppError 403 quando carrinho pertence a outro usuário", async () => {
            const purchase = makePurchase();
            const cart = makeShoppingCart({ userId: 99 });
        ...
        it("deve lançar AppError 404 quando pagamento não existe", async () => {
            ...
            vi.mocked(cartRepo.findById).mockResolvedValue(cart);
            vi.mocked(paymentRepo.findByPurchaseId).mockResolvedValue(null);
            await expect(paymentService.findByPurchaseId(1, 10)).rejects.toThrow(new AppError("Pagamento não encontrado", 404));
        });
    });
});
