...
describe("TicketService", () => {
    ...
    describe("findByPurchaseId", () => {
        ...
        it("deve lançar AppError 404 quando a compra não existe", async () => {
            ...
            await expect(ticketService.findByPurchaseId(99, 1)).rejects.toThrow(new AppError("Compra não encontrada", 404));
        ...
        it("deve lançar AppError 403 quando o carrinho não pertence ao usuário", async () => {
            ...
            await expect(ticketService.findByPurchaseId(10, 1)).rejects.toThrow(new AppError("Acesso negado", 403));
        });
    ...
    describe("validate", () => {
        ...
        it("deve lançar AppError 404 quando o código do ticket não existe", async () => {
            ...
            await expect(ticketService.validate("INVALIDO")).rejects.toThrow(new AppError("Ticket inválido", 404));
        ...
        it("deve lançar AppError 409 quando o ticket já foi utilizado", async () => {
            ...
            vi.mocked(ticketRepo.findByCode).mockResolvedValue(ticket);
            await expect(ticketService.validate("CODE-ABC")).rejects.toThrow(AppError);
            try {
    ...
    describe("findByCode", () => {
        ...
        it("deve lançar AppError 404 quando o código não existe", async () => {
            vi.mocked(ticketRepo.findByCode).mockResolvedValue(null);
            await expect(ticketService.findByCode("X")).rejects.toThrow(new AppError("Ticket não encontrado", 404));
        });
    });
});
