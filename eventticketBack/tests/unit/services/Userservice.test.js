...
describe("UserService", () => {
    ...
    describe("getUserById", () => {
        ...
        it("deve lançar AppError 404 quando o usuário não existe", async () => {
            ...
            await expect(userService.getUserById(99)).rejects.toThrow(new AppError("Usuário não encontrado", 404));
        });
    });
    describe("updateUser", () => {
        ...
        it("deve fazer hash da senha quando passwordHash é fornecido", async () => {
            ...
            vi.mocked(bcrypt.hash).mockResolvedValue("novo_hash");
            ...
            expect(userRepo.update).toHaveBeenCalledWith(1, expect.objectContaining({ passwordHash: "novo_hash" }));
        });
        ...
        it("deve lançar AppError 404 quando o usuário não existe", async () => {
            vi.mocked(userRepo.findById).mockResolvedValue(null);
            await expect(userService.updateUser(99, { name: "X" })).rejects.toThrow(new AppError("Usuário não encontrado", 404));
        });
        it("deve lançar AppError 400 quando o e-mail já pertence a outro usuário", async () => {
            ...
            await expect(userService.updateUser(1, { email: "outro@email.com" })).rejects.toThrow(new AppError("Email já cadastrado", 400));
    ...
    describe("createUserWithRole", () => {
        it("deve criar usuário com role personalizado", async () => {
            vi.mocked(userRepo.findByEmail).mockResolvedValue(null);
            vi.mocked(bcrypt.hash).mockResolvedValue("hash_admin");
            ...
            expect(userRepo.createWithRole).toHaveBeenCalledWith(expect.objectContaining({ passwordHash: "hash_admin", role: "ADMIN" }));
            expect(result).toEqual({
                id: created.id,
                name: created.name,
                email: created.email,
