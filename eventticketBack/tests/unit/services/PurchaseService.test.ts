...
import type { IShoppingCartCreate, IShoppingCart } from "../../../src/interfaces/shoppingCartInterface.ts";
...
const makeShoppingCartRepository = (): IShoppingCartRepository => ({
    ...
    findById: vi.fn(),
    update: vi.fn(),
    create: function (shoppingCart: IShoppingCartCreate): Promise<IShoppingCart> {
...
describe("PurchaseService", () => {
  ...
  describe("checkout", () => {
    it("deve lançar AppError 404 quando carrinho não existe", async () => {
      ...
      await expect(purchaseService.checkout(10)).rejects.toThrow(
    ...
    it("deve lançar AppError 400 quando carrinho está vazio", async () => {
      ...
      await expect(purchaseService.checkout(10)).rejects.toThrow(
        new AppError("Carrinho está vazio", 400),
      );
    ...
    it("deve lançar AppError 409 quando assento não está disponível", async () => {
      ...
      await expect(purchaseService.checkout(10)).rejects.toThrow(
        new AppError(`Assento ${item.seatId} não está mais disponível`, 409),
      );
    ...
    it("deve validar todos os assentos antes de criar compra", async () => {
      ...
      await expect(purchaseService.checkout(10)).rejects.toThrow(
        new AppError(`Assento ${item2.seatId} não está mais disponível`, 409),
      );
  ...
  describe("findById", () => {
    ...
    it("deve lançar AppError 404 quando compra não existe", async () => {
      ...
      await expect(purchaseService.findById(99, 10)).rejects.toThrow(
    ...
    it("deve lançar AppError 403 quando carrinho não existe", async () => {
      ...
      await expect(purchaseService.findById(1, 10)).rejects.toThrow(
